...
<body>
    ...
    <div class="practice-layout">
        ...
        <main class="practice-content">
            ...
            @php $skills = ['reading' => 'Reading', 'listening' => 'Listening', 'writing' => 'Writing', 'speaking' => 'Speaking']; @endphp

            @foreach($skills as $skillKey => $skillLabel)
                <div class="skill-panel {{ $loop->first ? 'active' : '' }}" data-panel="{{ $skillKey }}">
                    <div class="practice-grid">
                        @php $exams = $examsBySkill[$skillKey] ?? collect(); @endphp

                        @if($exams->isEmpty())
                            {{-- Fallback static samples when no published exams --}}
                            @if($skillKey === 'reading')
